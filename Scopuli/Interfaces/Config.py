...
class Config:
    ...
    xml_web = None
    configuration = None
    configuration_web = None
    
    def __init__(self, use_web=False, path="./config.xml"):
        """
        ...
        if use_web:
            ...
            for xml_file_web in self.xmr_files_web:
                if os.path.isfile(xml_file_web):
                    self.xml_file_web = xml_file_web
    ...
    def save(self):
        ...
        io_writer.writelines(self.xml.toprettyxml(indent="", newl=""))
        io_writer.close()

        if self.xml_web:
            io_writer = open(self.xml_file_web, "w")
            io_writer.writelines(self.xml_web.toprettyxml(indent="", newl=""))
            io_writer.close()
